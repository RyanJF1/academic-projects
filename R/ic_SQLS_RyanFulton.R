...
StartHere("Ryan Fulton", "IC", date())

ic1 = sqldf("SELECT zp.zipcode, SUM(irs.n1)
            FROM zp
            JOIN irs ON zp.zipcode=irs.zipcode
            WHERE zp.zipcode='30307'")
ic1

ic2 = sqldf("SELECT zp.state, zp.city, SUM(n1)
            FROM zp
            JOIN irs ON zp.zipcode=irs.zipcode
            WHERE zp.city='DULUTH' AND zp.state='GA'")
ic2

ic3 = sqldf("SELECT zp.state, SUM(n1)
            FROM zp
            JOIN irs ON zp.zipcode=irs.zipcode
            WHERE zp.state='TX'")
ic3

ic4 = sqldf("SELECT zp.state, zp.city, (SUM(ataxliab)*1000.0)
            FROM zp
            JOIN irs ON zp.zipcode=irs.zipcode
            WHERE zp.city='SAVANNAH' AND zp.state='GA'")
ic4

ic5 = sqldf("SELECT zp.state, zp.city, (SUM(acontrib)*1000.0)
            FROM zp
            JOIN irs ON zp.zipcode=irs.zipcode
            WHERE zp.city='DULUTH' AND zp.state='GA'")
ic5
